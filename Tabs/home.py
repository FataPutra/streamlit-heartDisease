...
def app():
    dh, x, y = load_data()
    st.title("Infomarsi Dataset Yang Kami Gunakan Untuk Prediksi Penyakit Jantung Koroner (Heart Disease)")
    ...
    st.subheader(
        "Petunjuk Mengenai Informasi Atribut Dataset Yang Digunakan Untuk Menggunakan Aplikasi Ini")
    st.markdown("1. **Age (Usia):** Usia pasien dalam tahun.")
    st.markdown(
        "2. **Sex (Jenis Kelamin):** Jenis kelamin pasien. Nilai 0 menandakan perempuan, dan nilai 1 menandakan laki-laki.")
    st.markdown(
        "3. **Chest Pain Type (Tipe Nyeri Dada):** Tipe nyeri dada yang dialami oleh pasien. Terdapat 4 nilai yang mungkin:")
    ...
    st.markdown(
        "4. **Resting Blood Pressure (Tekanan Darah Istirahat):** Tekanan darah pasien dalam keadaan istirahat (dalam mmHg).")
    st.markdown(
        "5. **Serum Cholestoral (Kolesterol Serum):** Kadar kolesterol serum pasien dalam mg/dl.")
    ...
    st.markdown(
        "   - 2: Memperlihatkan adanya hipertrofi ventrikel kiri yang pasti menurut kriteria Estes.")
    ...
    st.dataframe(dh)
    st.markdown(
        "Berikut merupakan tampilan dataset HeartDisease dengan atribut yang telah dijelaskan sebelumnya.")
    ...
    fig.add_trace(go.Pie(labels=dh['target'].value_counts().keys(),
                         values=dh['target'].value_counts(
    ).values, textfont=dict(size=16),
    ...
    st.markdown(
        "- Pada sumbu x (horizontal), kita menggunakan atribut 'age' yang merepresentasikan usia pasien.")
